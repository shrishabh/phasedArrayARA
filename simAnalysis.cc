...
int main(int argc, char **argv) {    // this is for manual power threshold value
  ...
  // }
  if (argc != 4) {
    cout<<"Wrong number of arguemnts."<<endl;
    cout<<"Usage ./simAnalysis <input file> <output directory> <run number>"<<endl;
    return 0;
  }
  ...
  // else { // no mode for argc > 2!
     ...
     string runNo = string (argv[3]);
  // }
  TFile *outputFile = new TFile((outputdir+"result."+runNo+".root").c_str(),"RECREATE");
  //TODO : Modify this code to take in multiple input files using TChain - Done
...
*/
  ...
  double posnuTheta;
  double posnuPhi;
  double avgSNR[settings->NNU];

  double distanceFromOrigin[settings->NNU];
  ...
  double snr; double radius; double dist; double depth;
  TTree *tree = new TTree("eventTree","Tree for variables");
  tree->Branch("SNR",&snr,"SNR/D");
  tree->Branch("radius",&radius,"radius/D");
  tree->Branch("dist",&dist,"dist/D");
  tree->Branch("depth",&depth,"depth/D");

  TTree *tree2 = new TTree("overAllTree","Tree for variables");
  tree2->Branch("numPas",&nnu_pass,"numPass/I");

  double wf_time_offset = -450.;// in ns
  double offSetZ = (double)detector->stations[0].GetZ();
  ...
  //cout<<"OffSet Z = "<<(long)offSetZ<<endl;
...
//            if ( event->Nu_Interaction[0].pickposnu ) {
              ...
              posnuZ[nnu_pass] = event->Nu_Interaction[0].posnu.GetZ() - offSetZ;
              //radial distance
              radDistance = sqrt(posnuX*posnuX + posnuY*posnuY );
              avgSNR[nnu_pass] = report->stations[0].strings[0].averageSNR;
              ...
              distanceFromOrigin[nnu_pass] = report->stations[0].strings[0].antennas[3].Dist[0];
              //cout<<"Distance  = "<<distanceFromOrigin[nnu_pass]<<endl;

              //TO GET THE WAVEFORM INFORMATION, READ THE CODE BELOW
              // double getx[bin];
              // double gety[bin];
              // cout<<"Bin "<<bin<<endl;
              // int numAntennas = 7;
            ...
            //   cout<<"Average SNR "<<snr[nnu_pass]<<endl;
              ...
              snr = avgSNR[nnu_pass];
              radius = posnuR[nnu_pass];
              dist = distanceFromOrigin[nnu_pass];
              depth = posnuZ[nnu_pass];
  ...
  gr_radiusDepth = new TGraph(numEvents,posnuR,posnuZ);
  gr_radiusDepth->SetTitle("Radius vs Depth for triggered Events");
  gr_radiusDepth->GetXaxis()->SetLimits(10,8000);
  ...
  //c1->Print("/home/rkhandelwal/public_html/phasedArrayPlots/temp/PAradiusDepthE18.pdf");
  gr_radiusDepth->Write("radius_depth");
  //avgSNR->Write();
  //TCanvas *c2 = new TCanvas("c2","A Simple Graph Example",200,10,1000,800);
  TGraph *gr_distDepth;
  //cout<<"Got here as well"<<endl;
  gr_distDepth = new TGraph(numEvents,distanceFromOrigin,posnuZ);
  gr_distDepth->SetTitle("Distance Vs Depth for triggered Events");
  gr_distDepth->GetXaxis()->SetLimits(10,8000);
  gr_distDepth->GetYaxis()->SetTitle("Z (depth, m)");
  gr_distDepth->SetMarkerColor(4);
  gr_distDepth->SetMarkerSize(0.5);
  gr_distDepth->SetMarkerStyle(20);
  gr_distDepth->GetHistogram()->SetXTitle("Distance from Surface (m)");
  gr_distDepth->GetYaxis()->SetTitleOffset(1.2);
  //cout<<"Got here"<<endl;
  gr_distDepth->Draw("ap");

  //c1->Print("/home/rkhandelwal/public_html/phasedArrayPlots/temp/PAdistDepthE18.pdf");
  gr_distDepth->Write("distance_depth");

  //TCanvas *c1 = new TCanvas("c1","A Simple Graph Example",200,10,1000,800);
  TGraph *gr_snrRadius;
  //cout<<"Got here as well"<<endl;
  gr_snrRadius = new TGraph(numEvents,posnuR,avgSNR);
  gr_snrRadius->SetTitle("Radius vs Average SNR for triggered Events");
  gr_snrRadius->GetXaxis()->SetLimits(10,8000);
  gr_snrRadius->GetYaxis()->SetTitle("Average SNR");
  gr_snrRadius->SetMarkerColor(4);
  gr_snrRadius->SetMarkerSize(0.5);
  gr_snrRadius->SetMarkerStyle(20);
  gr_snrRadius->GetHistogram()->SetXTitle("Radius (m)");
  gr_snrRadius->GetYaxis()->SetTitleOffset(1.2);
  //cout<<"Got here"<<endl;
  gr_snrRadius->Draw("ap");

  //c1->Print("/home/rkhandelwal/public_html/phasedArrayPlots/temp/PAsnrRadiusE18.pdf");
  gr_snrRadius->Write("snr_radius");

  //TCanvas *c1 = new TCanvas("c1","A Simple Graph Example",200,10,1000,800);
  TGraph *gr_snrDistance;
  //cout<<"Got here as well"<<endl;
  gr_snrDistance = new TGraph(numEvents,distanceFromOrigin,avgSNR);
  gr_snrDistance->SetTitle("Distance vs Average SNR for triggered Events");
  gr_snrDistance->GetXaxis()->SetLimits(10,10000);
  gr_snrDistance->GetYaxis()->SetTitle("Average SNR");
  gr_snrDistance->SetMarkerColor(4);
  gr_snrDistance->SetMarkerSize(0.5);
  gr_snrDistance->SetMarkerStyle(20);
  gr_snrDistance->GetHistogram()->SetXTitle("Distance (m)");
  gr_snrDistance->GetYaxis()->SetTitleOffset(1.2);
  //cout<<"Got here"<<endl;
  gr_snrDistance->Draw("ap");

  //c1->Print("/home/rkhandelwal/public_html/phasedArrayPlots/temp/PAsnrDistanceE18.pdf");
  gr_snrDistance->Write("snr_distance");

  // TCanvas *c1 = new TCanvas("c1","A Simple Graph Example",200,10,1000,800);
  // TGraph2D *gr = new TGraph2D(numEvents,distanceFromOrigin,posnuZ,avgSNR);
  // gr->SetTitle("Distance Vs Depth for triggered Events");
  // gr->GetXaxis()->SetLimits(10,10000);
  // gr->GetYaxis()->SetTitle("Average SNR");
  // gr->GetHistogram()->SetXTitle("Distance (m)");
  // gr->GetYaxis()->SetTitleOffset(1.2);
  // gr->Draw("PCOLZ");
  // gr->Write("dist_depth_snr");
  // c1->Print("PAsnrDistanceE18.pdf");
  //
  // TCanvas *c2 = new TCanvas("c2","A Simple Graph Example",200,10,1000,800);
  // TGraph2D *gr1 = new TGraph2D(numEvents,posnuR,posnuZ,avgSNR);
  // gr1->SetTitle("Radius Vs Depth for triggered Events");
  // gr1->GetXaxis()->SetLimits(10,10000);
  // gr1->GetYaxis()->SetTitle("Average SNR");
  // gr1->GetHistogram()->SetXTitle("Radius (m)");
  // gr1->GetYaxis()->SetTitleOffset(1.2);
  // gr1->Draw("PCOLZ");
  // gPad->SetTheta(90.); gPad->SetPhi(0.001);
  // c2->Print("PAsnrRadius.pdf");
  // gr1->Write("radius_depth_snr");
}
